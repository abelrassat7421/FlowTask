...
class StartWindow:
    ...
    def capture_trajectory(self, sampling_rate=100): 
        ...
        img.paste(self.image_tar_resized, (round(self.lbl_target.winfo_rootx()), round(self.lbl_target.winfo_rooty())), mask=a)
        #img.paste(self.image_tar, (self.lbl_target.winfo_rootx(), self.lbl_target.winfo_rooty()), mask=a)
        print(f"DEBUG: {self.lbl_target.winfo_rootx(), self.lbl_target.winfo_rooty()}")
        print(f"DEBUG: {self.lbl_target.winfo_width(), self.lbl_target.winfo_height()}")
        print(f"DEBUG: {self.screen_width, self.screen_height}")
    ...
    def record_and_draw_last_traj_pt(self, draw, coordinates, prev_x, prev_y, img):
        if self.recording == True:
            ...
            print("DEBUGG: LIVE Trajectory", (x, y))
    ...
    def show_target_based_on_pos(self):
        ...
        if y < (self.TriggerPos[self.trial_counter]) * self.screen_height:
            ...
            print(f"DEBUG post: {self.lbl_target.winfo_rootx() - self.decx, self.lbl_target.winfo_rooty() - self.decy}")
            print(f"DEBUG post: {self.lbl_target.winfo_width(), self.lbl_target.winfo_height()}")
    ...
    def move_next_trial(self):
        ...
        # remove mouse when not in trial
        if self.Inverted:
